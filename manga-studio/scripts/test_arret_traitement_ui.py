# -*- coding: utf-8 -*-
"""Banc v2.66.0 : « ✖ Arrêter » un lot / une narration depuis le DETAIL DE L'ACTIVITE (reutilise l'interruption v2.12.0,
SANS changer de mode). Derive de test_interruption_ui.py : meme faux lot, meme restauration. 0 $.
...
try:
    ...
    with sync_playwright() as p:
        ...
        def dialogue(d):
            ...
            (d.dismiss if len(dialogues) == 1 else d.accept)()        # 1er : Annuler ; 2e : confirmer
        pg.on("dialog", dialogue)
        ...
        pg.evaluate("() => { $('actPanel').hidden = false; actRafraichir(); }"); pg.wait_for_timeout(2500)
        btn = pg.evaluate("() => [...document.querySelectorAll('#actListe [data-arret]')].map(b => b.dataset.arret + ':' + b.textContent.trim())")
        check("détail de l'activité : « ✖ Arrêter » sous le lot et sa narration", btn and all(x == "interrompre:✖ Arrêter" for x in btn), btn)
        # 1. Annuler
        pg.click("#actListe [data-arret='interrompre']"); pg.wait_for_timeout(1500)
        m = dialogues[0] if dialogues else ""
        check("confirmation : le lot listé UNE fois, « Arrêt propre », « Reprendre »", m.count("•") == 1 and "⚙ Lot" in m
              and "Arrêt propre" in m and "Reprendre" in m, m[:220])
        check("Annuler -> rien ne s'arrête, mode inchangé", lot.poll() is None and etape.poll() is None and reglages.lire()["mode"] == "cloud")
        # 2. confirmer
        pg.evaluate("() => actRendre()"); pg.click("#actListe [data-arret='interrompre']"); pg.wait_for_timeout(6000)
        time.sleep(0.5)
        check("Arrêter -> processus du lot ET de l'étape tués", lot.poll() is not None and etape.poll() is not None)
        check("… et le mode NE change PAS (reste ☁)", reglages.lire()["mode"] == "cloud")
