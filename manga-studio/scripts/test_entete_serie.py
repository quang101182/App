...
with sync_playwright() as p:
    ...
    for w, h in ((1280, 900), (360, 780)):
        ...
        vis = pg.eval_on_selector_all("#libNav .lib-actions > .btn", "bs => bs.filter(b => b.offsetParent).map(b => b.innerText.replace(/\\s+/g,' ').trim())")
        check("4 actions en vue", len(vis) == 4, vis)
        check("menu ferme au depart", not pg.is_visible("#libNav .lib-actions .menu-pan"))
        tops = pg.eval_on_selector_all("#libNav .lib-actions > .btn, #libNav .lib-actions .plus", "bs => bs.map(b => Math.round(b.getBoundingClientRect().top))")
        check("actions + ⋯ sur une seule rangee", len(set(tops)) == 1, tops)
        pg.click("#libNav .lib-actions .plus"); pg.wait_for_timeout(200)
        items = pg.eval_on_selector_all("#libNav .lib-actions .menu-pan > button", "bs => bs.filter(b => b.offsetParent).map(b => b.innerText.trim())")
        ...
        mp = boite(pg, "#libNav .lib-actions .menu-pan")
        check("menu entierement dans l'ecran", mp[0] >= 0 and mp[0] + mp[2] <= w + 0.5, mp)
        check("aria-expanded = true", pg.get_attribute("#libNav .lib-actions .plus", "aria-expanded") == "true")
        pg.screenshot(path=os.path.join(ICI, "entete_%d.png" % w))
        pg.keyboard.press("Escape"); pg.wait_for_timeout(150)
        check("Echap referme", not pg.is_visible("#libNav .lib-actions .menu-pan"))
        pg.click("#libNav .lib-actions .plus"); pg.wait_for_timeout(150)
        pg.mouse.click(5, h - 5); pg.wait_for_timeout(150)
        check("clic ailleurs referme", not pg.is_visible("#libNav .lib-actions .menu-pan"))
        pg.click("#libNav .lib-actions .plus"); pg.wait_for_timeout(150)
        pg.click("#libNav .lib-actions .plus"); pg.wait_for_timeout(150)
        check("2e clic sur ⋯ referme", not pg.is_visible("#libNav .lib-actions .menu-pan"))
