...
def main():
    ...
    stats, detail, tot_b, tot_c = {"cout": 0.0}, {}, 0, 0
    for d in chs:
        ...
        detail[d] = {"version": t.get("version"), "pages": len(pages), "blanches": blanches,
                     "restes": {str(k): v for k, v in restes.items()}}
        print("%-7s v%-7s %3d p. | blanches %2d %s%s" % (d, t.get("version"), len(pages), len(blanches), blanches,
              (" | textes non latins restes %d sur %d page(s)" % (n_c, len(restes))) if a.cjk else ""), flush=True)
    print("TOTAL : %d page(s) blanche(s)%s%s" % (tot_b, (" · %d texte(s) non latin(s) restant(s)" % tot_c) if a.cjk else "",
