...
2. on reste sur le 300 : une demande faite ailleurs apparait, puis passe « a jour » (copie de 3 pages : rendu de quelques secondes) ;
...
bo.copier_serie("one-punch-man", S, ["ch_300", "ch_301"], pages_max=3, avec=("serie.json",))   # 3 pages : une video en quelques secondes
try:
  with sync_playwright() as p:
    ...
    # 2. on RESTE sur le chapitre : une demande faite AILLEURS (autre appareil, lot) apparait, puis passe « a jour », sans ↻
    shutil.rmtree(vd, ignore_errors=True)
    api("/manga/video", {"entrees": [{"d": S + "/ch_300", "tag": tag}], "reglages": {"vitesse": 1, "sous": True, "camera": "cases"}})
    t0 = time.time(); vu = ""
    while time.time() - t0 < 30:
        pg.wait_for_timeout(1000); vu = etat()
        if "en attente" in vu or "fabrication" in vu or "à jour" in vu:
            break
    check("demande faite ailleurs -> visible dans le chapitre ouvert, sans ↻ (%.0f s)" % (time.time() - t0),
          "en attente" in vu or "fabrication" in vu or "à jour" in vu, vu[-60:])
    t0 = time.time()
    while time.time() - t0 < 180 and "à jour" not in etat():
        pg.wait_for_timeout(2000)
    check("... puis « à jour » quand elle est finie, sans ↻ (%.0f s)" % (time.time() - t0), "à jour" in etat(), etat()[-60:])
