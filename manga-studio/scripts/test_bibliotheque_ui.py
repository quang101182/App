...
with sync_playwright() as p:
    ...
    for w, h in ((1280, 900), (360, 780)):
        ...
        # v1.80.0 : bloc de capture repliable, etat garde -- v1.92.0 : REPLIE par defaut (ergonomie, Quang 22/09)
        check("1re visite : bloc de capture replie", pg.evaluate("() => !$('capBox').open"))
        pg.click("#capBox > summary"); pg.wait_for_timeout(200)
        pg.reload(); pg.wait_for_timeout(3000)
        check("deplie -> reste deplie apres rechargement", pg.evaluate("() => $('capBox').open"))
        pg.click("#capBox > summary"); pg.wait_for_timeout(200)
        pg.reload(); pg.wait_for_timeout(3000)
        check("replie -> reste replie apres rechargement", pg.evaluate("() => !$('capBox').open"))
