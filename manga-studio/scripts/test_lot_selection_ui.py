...
with sync_playwright() as p:
    ...
    for w, h in ((1280, 900), (360, 780)):
        ...
        # v2.31.0 : les outils de selection sont ranges dans « ⋯ Choisir » -- on l'ouvre, comme Quang
        pg.click("#lotSelbar .plus"); pg.wait_for_timeout(200)
        check("les 3 boutons sont visibles (⋯ Choisir ouvert)", all(pg.is_visible(x) for x in ("#lotTout", "#lotRien", "#lotPlage")))
        pg.keyboard.press("Escape")
        pg.evaluate("s => { const e = document.querySelector(s); if (!e) return; const d = e.closest('details'); if (d && !d.open) d.open = true; const m = e.closest('.menu-plus'); if (m && m.querySelector('.menu-pan').hidden) m.querySelector('.plus').click(); }", "#lotRien"); pg.click("#lotRien"); pg.wait_for_timeout(250)
        ...
        pg.evaluate("s => { const e = document.querySelector(s); if (!e) return; const d = e.closest('details'); if (d && !d.open) d.open = true; const m = e.closest('.menu-plus'); if (m && m.querySelector('.menu-pan').hidden) m.querySelector('.plus').click(); }", "#lotTout"); pg.click("#lotTout"); pg.wait_for_timeout(250)
        ...
        pg.click("#lotSelbar .plus"); pg.wait_for_timeout(150)                         # v2.31.0 : la plage est dans le menu
        pg.fill("#lotDe", ("%g" % LO).replace(".", ",")); pg.fill("#lotA", "%g" % HI); pg.evaluate("s => { const e = document.querySelector(s); if (!e) return; const d = e.closest('details'); if (d && !d.open) d.open = true; const m = e.closest('.menu-plus'); if (m && m.querySelector('.menu-pan').hidden) m.querySelector('.plus').click(); }", "#lotPlage"); pg.click("#lotPlage"); pg.wait_for_timeout(250)
        ...
        pg.locator("#suiviBox .prof-lot").screenshot(path=os.path.join(os.path.dirname(os.path.abspath(__file__)), "lot_sel_%d.png" % w))
