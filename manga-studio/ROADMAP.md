...
| **14 — APP INSTALLABLE sur le téléphone (Quang 22/09 08h36)** — **PROCHAINE après l'étape 7** | Chrome Android n'offre qu'un « raccourci » (barre d'adresse et bandeau de notification visibles) : l'app n'a pas les prérequis d'une PWA. À faire : `manifest.json` (display standalone, icônes 192/512, start_url `/manga`) + service worker minimal servis par le proxy 8190 sous `/manga/`. ⚠ Piège connu (mémoire `reference_pwa_derriere_authentification`) : si le tunnel est derrière une authentification, le manifeste et les icônes sont chargés SANS cookie → ils doivent rester libres, sinon l'installation échoue en silence. Vérifier sur le Samsung de test (`Page.getAppManifest` url non vide) | installer depuis Chrome = icône sur l'écran d'accueil, plein écran sans barre | installation réelle vérifiée |
