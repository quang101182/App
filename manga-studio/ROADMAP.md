...
|  | 🧪 **ESSAI 26/09 22h (Quang : « comme dans StoryVoice […] pour voir ce que ça donnerait »)** — `scripts/essai_voix_personnages.py` v0.2.0, HORS app : OPM ch.7 p.2-6, 38 répliques. 1 appel Gemini 3.6 Flash (pages traduites + bulles numérotées + fiche narration) → distribution (voix Gemini TTS au catalogue + fiche de jeu) + locuteur ET ton PAR RÉPLIQUE ; voix `gemini-2.5-flash-tts` (`/api/gcptts`, `input.prompt`) ; vidéo de contrôle (bulle encadrée + « Perso (ton) »). **0,13 $** (distribution 0,03 + voix 0,06-0,10), ~4 min d'audio. StoryVoice ne règle que la VOIX par personnage (ton = par chapitre) : ici le ton est par réplique. ⚠️ **Piège payé** : une consigne longue (« Tu doubles X. <fiche>. Réplique à jouer : … ») est LUE À VOIX HAUTE (« Quoi ? » = 11 s) → consigne courte « Dis ceci d'un ton X, en français. » = **0 fuite / 6 puis 0 / 37** (contrôle Whisper par réplique, refaite si fuite) ; la fiche ne va PAS à la voix. Limites vues : personnages non nommés affichés « p2 / p3 », monologues de 55-68 s sur un seul plan, attributions NON vérifiées bulle à bulle. Sortie : `<données>/one-punch-man/ch_7/essai_voix/essai.mp4`. ~~**Déclencheur de suite** : verdict de Quang à l'écoute.~~ → **Verdicts 26/09 23h13-23h36** : scène courte homme+femme (OPM ch.6 p.5-7) ; **ElevenLabs v3 « clairement au-dessus »** de Gemini TTS et d'ElevenLabs v2 (tons = balises anglaises `[mocking]` dans le texte, 0 prononcée). Atelier local `essai_voix_atelier.py` (:8196) : voix/caractère/intensité/vitesse par perso, qui/ton/lire par réplique, ▶, interrupteur tons. Coût mesuré : attribution ~0,004 $/page, 153 car. de dialogue/page (476 p.) → quota gratuit EL 10 000 crédits ≈ 35-65 p./mois. Contenu adulte (texte) : 24/24 acceptés, 0 censure. **Orientation Quang** : mode « Dialogues » SÉPARÉ de la narration, corrections manuelles (texte compris) avant/après, **AUCUN moteur de secours** (quota EL épuisé = bloqué + crédits affichés), tons par réplique gardés. **Déclencheur** : feu vert de Quang pour la MAQUETTE (pas de code avant validation). |
