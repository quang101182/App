...
| **18 — CAPTURE AVANCÉE : PLUSIEURS CHAPITRES (Quang 22/09 10h43)** — après | Définir le nombre de chapitres à capturer à partir de celui où je suis, ou « du ch. X au ch. Y » ; certains sites enchaînent seuls le chapitre suivant (MangaDex le fait : constaté par la session GLM le 21/09) — **à vérifier site par site avant de coder** (MangaDex, MANGA Plus…). | une série entière en un geste | N chapitres capturés sans intervention |
...
| **19 — SITES VALIDÉS pour la capture (Quang 22/09 10h47)** — avec 18 | Dans la section capture : la **liste des sites fonctionnels**, chacun avec son état mesuré (capture OK ? enchaînement automatique des chapitres possible ? date et chapitre du dernier contrôle) et un **lien direct** vers le site : un toucher l'ouvre dans le navigateur du téléphone (Quang passe surtout par le smartphone) ; de là, « Partager → Manga Studio » (étape 17). Aujourd'hui **2 sites** : **MangaDex** (page par page ; OPM 301 19/19) et **MANGA Plus** (capture dans le navigateur ; Claymore 1 62/62, Boruto 52/52 — la voie API/jeton est morte, README manga-fetch). **Règle : tout nouveau site passe par Claude** (capture réelle d'un chapitre + test de l'enchaînement) AVANT d'entrer dans la liste ; un site qui casse est marqué, pas retiré en silence. La liste vit dans un fichier versionné (pas en dur dans le HTML). | savoir où capturer, y aller en un geste | liste affichée, lien ouvert depuis le téléphone |
