...
| ✅ **5-bis — VIDÉO, compléments FAITS v1.95.1 (22/09 09h15)** (demandes Quang 09h00 → 09h10) | (a) **« Précédemment… » en tête de la vidéo si la case 📜 est cochée** (le réglage suit la case comme les sous-titres ; empreinte « le Précédemment… a été refait » ; clé ajoutée SEULEMENT si demandé → les vidéos d'avant ne passent pas 🟠) ; sous-titres longs réduits (46 → ≥ 30 px selon la longueur : le résumé débordait du bandeau). (b) **Voix de la vidéo** : menu quand le chapitre a ≥ 2 narrations avec voix (défaut = voix de la vidéo existante, sinon la plus récente ; mémorisé par chapitre). (c) **Nom du fichier téléchargé** lisible : « Série - ch001 - Voix - VO/FR - karaoké - musique 25 % - précédemment - 1,15x - date heure.mp4 » (+ nom ASCII de secours) — vérifié en VRAI téléchargement sur le Samsung. (d) pastille des coûts à DROITE sur téléphone (caméra au centre, Fold 8) et raccourcie. Bancs : `test_video_voix_ui.py` 25/25 (dont une vraie vidéo OPM ch.301 par la file), `test_precedemment_ui.py` 37/37, `test_karaoke_ui.py` 28/28. | | ✅ |
| **15 — TÉLÉCHARGER UN GROUPE DE VIDÉOS (Quang 22/09 09h17)** | Dans le panneau Vidéos de la série : tout est coché par défaut, ou je choisis ; prévu pour BEAUCOUP de chapitres. Existe déjà (v1.93) : cases + « ⬇ la sélection » un par un à 1,5 s d'écart. À faire : « tout cocher / décocher », sélection par plage (ch. X à Y) ou par tome, et au-delà de ~5 vidéos une archive unique (ZIP en flux par le proxy, sans recompression) plutôt que N téléchargements (le téléphone en bloque une partie). À mesurer : taille (~160 Mo/chapitre de 60 p.) → avertir au-delà de 1-2 Go. | ne plus cliquer chapitre par chapitre | 20 chapitres en un geste, téléphone compris |
| **16 — CHAPITRE PRÉCÉDENT / SUIVANT en un geste (Quang 22/09 09h19)** | Boutons rapides ⏮ ch. / ch. ⏭ dans le LECTEUR (enchaîner sans sortir, et en fin de chapitre proposer le suivant) ET sur la fiche d'un chapitre (ouvrir le voisin sans repasser par la liste). Chapitres de la même série, dans l'ordre des numéros ; un voisin sans narration avec voix = grisé avec la raison. | enchaîner la lecture | aller-retour ch. N ↔ N+1 sans sortir, PC + téléphone |
