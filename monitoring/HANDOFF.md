...
- **✅ LOT 6 / P3b2 — DictoKey Appareils (Claude, 16/07, v2.5.0-p3b2)**.
  ...
  - 🐛 **BUG PROD CORRIGÉ en v2 (feu vert Quang 16/07, v2.6.0-p3b2) — divergence assumée** : `getDeviceCategory()` (prod l.3813-3822) teste `pro` puis `effectiveTier === 'premium'` et **oublie le cas `tier === 'premium'` en KV** → les appareils passés premium explicitement (`/device/restore`, `/device/update-tier`) retombaient dans « free ». Concrètement : pills premium **16** alors que le summary comptait **18**, et ces 2 appareils **disparaissaient dès qu'on décochait « free »**. Corrigé en v2 : `if (kvTier === 'premium' || d.effectiveTier === 'premium') → premium`. Vérifié : pills premium **16 → 18** (= summary), free **413 → 411**, filtrer premium seul → **18 appareils**. Summary/fantômes inchangés. ⚠ La prod garde le bug jusqu'à la bascule P5 (interdit d'y toucher).
